...
class MAML:
	...
	def _inner_loop(self, images, labels, train):
		...
		for i in range(self._num_inner_steps):
			...
			gradients = torch.autograd.grad(
				...
				create_graph=train
			) # keep the computation graph to backprop through the gradient steps
	...
	def _outer_step(self, task_batch, train):
		...
		Args:
			task_batch (tuple): List[Tuple[Tensor]], each tuple contains [img_sup, label_sup, img_qry, label_qry]
			train (bool): whether we are training or evaluating
		...
		for task in task_batch:
			...
			accuracy_query = util.score(preds_query, labels_query)
		...
		accuracy_support = np.mean(np.array(accuracies_support_batch), axis=0)
		accuracy_query = np.mean(accuracy_query_batch)
		return outer_loss, accuracy_support, accuracy_query
