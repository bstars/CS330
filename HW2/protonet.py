...
class ProtoNet:
	...
	def _step(self, task_batch):
		"""
		Computes ProtoNet mean loss and mean accuracy on a batch of tasks.

		:param task_batch: List[Tuple[Tensor]],
		    each tuple contains [img_sup, label_sup, img_qry, label_qry] where


		:return:
		:rtype:
		"""
		# img_sup, label_sup, img_qry, label_qry = task_batch
		# print(img_sup.shape, label_sup.shape, img_qry.shape, label_qry.shape)
		loss_batch = []
		...
		for task in task_batch:
			...
			embed_sup = self._network(images_support)
			embed_qry = self._network(images_query)

			# compute the prototype for each class
			unique_labels = torch.unique(labels_support)
			prototypies = []
			for l in unique_labels:
				idx = torch.where(labels_support == l)[0]
				prototypies.append( torch.mean(embed_sup[idx], dim=0) )
			prototypies = torch.stack(prototypies)


			# query set loss and accuracy
			d = torch.cdist(embed_qry, prototypies) # [num_qry, num_prototypes]
			loss = F.cross_entropy(-d, labels_query)
			acc_qry = util.score(-d, labels_query)
			loss_batch.append(loss)
			accuracy_query_batch.append(acc_qry)

			# support accuracy
			d = torch.cdist(embed_sup, prototypies)
			acc_sup = util.score(-d, labels_support)
			accuracy_support_batch.append(acc_sup)
		return (
